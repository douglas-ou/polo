...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`Polo AIs Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Polo AIs Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Polo AIs Docs server is not connected. Tool '${name}' unavailable.`);
  }
