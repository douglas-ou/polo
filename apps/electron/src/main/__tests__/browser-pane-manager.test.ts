...
let mockWebContentsIdCounter = 0

function createMockWebContents() {
  const listeners: Record<string, Function[]> = {}
  let currentUrl = 'about:blank'
  return {
    id: ++mockWebContentsIdCounter,
...
describe('BrowserPaneManager', () => {
  ...
  it('destroys child popups when parent instance is destroyed', () => {
    ...
    // did-create-window passes (window, details) without an Electron event
    // object, so call registered listeners directly instead of using _emit
    // (which prepends an empty event object)
    const handlers = instance.pageView.webContents._listeners['did-create-window'] || []
    for (const h of handlers) h(popupWindow, { url: 'https://accounts.google.com/signin' })
